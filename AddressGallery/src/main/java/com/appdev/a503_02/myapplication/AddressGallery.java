package com.appdev.a503_02.myapplication;
...
public class AddressGallery extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_address_gallery);
        ...
        //os가 6.0 이상일 때 동적인 권한 요청 - 연락처
        if(ContextCompat.checkSelfPermission(this, Manifest.permission.READ_CONTACTS)
                !=PackageManager.PERMISSION_GRANTED){
            ActivityCompat.requestPermissions(this, new String[]{Manifest.permission.READ_CONTACTS}, 100);
        }

        //os가 6.0 이상일 때 동적인 권한 요청 - 외부저장장치
    ...
    public void onClick(View v){
        //연락처 클릭, contactBtn이 인스턴스 변수가 아니라면, if(v.getId()==R.id.lab2_contacts)로 해주어야 한다.
        if(v == contactBtn){
            Intent intent = new Intent(Intent.ACTION_PICK);
            intent.setData(Uri.parse("content://com.android.contacts/data/phones"));

            //엑티비티를 화면에 출력하고 출력된 엑티비티가 종료되면 10이라는 값과 onActivityResult 메소드를 호출한다.
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        private void insertImageView(String filepath){
            ...
            if(filepath.equals("") == false){
                ...
                int sampleSize = 1;
                if(width > reqWidth) { //화면 사이즈보다 이미지가 클 경우

                    //이미지의 크기가 화면 크기보다 크다면 이미지를 축소하기 위해서 나누기 해서 그 비율을 sampleSize로 대입
        ...
        private String getFilePathFromDocumentUrl(Context context, Uri uri){
            ...
            String type = split[0];
            Uri contentUri = null;
            //NullPointerExceptiond을 방지하기 위해서 상수를 가지고 비교 즉, 상수.equals(입력받는 값)
